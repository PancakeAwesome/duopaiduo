...
function info(){
	$.ajax({
		...
		success : function(data) {
			...
			if(data.status==1){
				$("#logistNo").html('<input id="logistNos" type="text" value="'+(data.logistNo==null?'':data.logistNo)+'"/>');
				$("#logistMoney").html('<input id="logistMoneys" type="text" value="'+(data.logistMoney==null?'':data.logistMoney)+'" />');
			}
			if(data.status!=1){
				...
				if(data.status==1){
					...
					$("#typename").html('<input type="button" onclick="upstatus(2)" value="确认发货">');
				}
				if(data.status==2){
					$("#typename").html('<input type="button" onclick="upstatus(3)" value="确认收款"><input type="button" onclick="upstatus(4)" value="确认退货">');
					$("#btn_wuLiu").removeClass("none");
				}
				if(data.status==3){
					$("#typename").html('<h2>已收款</h2>');
					$("#btn_wuLiu").removeClass("none");
				}
				if(data.status==4){
					$("#typename").html('<h2>已退货</h2>');
					$("#btn_wuLiu").removeClass("none");
				}
			},
			error : function() {
