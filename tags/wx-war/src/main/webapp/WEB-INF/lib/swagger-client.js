  ...
  var obj = {
    ...
    on: {
      ...
      response: function(resp) {
        var responseObj = resp.obj.data || JSON.parse(resp.data.data);
...
//  log(response);
//  alert(2)
  for (var i = 0; i < response.data.apis.length; i++) {
    var api = response.data.apis[i];
    if (api.operations) {
  ...
  if (isApi) {
    ...
    for (k = 0; k < response.data.apis.length; k++) {
      resource = response.data.apis[k];
      res = new SwaggerResource(resource, this);
      this.apis[res.name] = res;
      this.apisArray.push(res);
    }
...
SwaggerResource.prototype.addApiDeclaration = function (response) {
  ...
  if (response.data.apis) {
    for (var i = 0 ; i < response.data.apis.length; i++) {
      var endpoint = response.data.apis[i];
...
var SwaggerModelProperty = function (name, obj, model) {
  this.name = name;
  this.dataType = obj.type.absoluteType || obj.dataType.absoluteType || obj.$ref.absoluteType;
  //alert(this.dataType);
  this.isCollection = this.dataType && (this.dataType === 'array' || this.dataType === 'list' || this.dataType === 'set');
